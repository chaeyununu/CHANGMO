...
function analyze() {
  ...
  T.sub=Math.min(sv*1.35,1); T.bass=Math.min(bv*1.25,1); T.mid=Math.min(mv*1.15,1);
  T.high=Math.min(hv*1.10,1); T.air=Math.min(av*1.05,1);
  T.energy=Math.min(sv*.38+bv*.30+mv*.18+hv*.09+av*.05,1);

  beatCool=Math.max(0,beatCool-1);
  if (sv>.14&&sv-prevSub>.08&&beatCool===0) { S.beat=1.0; beatCool=7; }
  prevSub=sv;

  bassHitCool=Math.max(0,bassHitCool-1);
  if (bv>.20&&bv-prevBass>.10&&bassHitCool===0) {
    LQ.bassShockTarget=1.0; bassHitCool=12;
  }
  prevBass=bv;
}

function smooth() {
  S.sub    +=(T.sub    -S.sub)   *SM.sub;
  S.bass   +=(T.bass   -S.bass)  *SM.bass;
  S.mid    +=(T.mid    -S.mid)   *SM.mid;
  S.high   +=(T.high   -S.high)  *SM.high;
  S.air    +=(T.air    -S.air)   *SM.air;
  S.energy +=(T.energy -S.energy)*SM.energy;
  S.beat   +=(0        -S.beat)  *SM.beat;

  LQ.subPressureTarget = S.sub * 0.92 + S.bass * 0.20;
  LQ.subPressure += (LQ.subPressureTarget - LQ.subPressure) * 0.62;
  LQ.subPressure += (0 - LQ.subPressure) * 0.09;

  LQ.midDensityTarget = S.mid * 0.78 + S.bass * 0.35;
  LQ.midDensity += (LQ.midDensityTarget - LQ.midDensity) * 0.28;

  LQ.highShimmerTarget = S.high * 0.72 + S.air * 0.48;
  LQ.highShimmer += (LQ.highShimmerTarget - LQ.highShimmer) * 0.36;
  LQ.highShimmer += (0 - LQ.highShimmer) * 0.06;

  LQ.airGlowTarget = S.air * 0.65 + S.high * 0.52;
  LQ.airGlow += (LQ.airGlowTarget - LQ.airGlow) * 0.12;

  LQ.surfaceTensionTarget = S.high * 0.58 + S.air * 0.55;
  LQ.surfaceTension += (LQ.surfaceTensionTarget - LQ.surfaceTension) * 0.40;
  LQ.surfaceTension += (0 - LQ.surfaceTension) * 0.09;

  LQ.bassShock += (LQ.bassShockTarget - LQ.bassShock) * 0.62;
  LQ.bassShock += (0 - LQ.bassShock) * 0.14;
  LQ.bassShockTarget *= 0.55;

  LQ.refractionPhase += 0.014 + S.energy * 0.035 + LQ.highShimmer * 0.024;

  // ── Pool onset detector: 조용→큰소리 순간 감지 ──
  // baseline은 느리게 추적 (약 2~3초 평균)
  _poolEnergySmooth += (S.energy - _poolEnergySmooth) * 0.008;
  _poolOnsetCool = Math.max(0, _poolOnsetCool - 1);
  const _energyRise = S.energy - _poolEnergySmooth;
  // 에너지가 baseline 대비 급등 + 쿨다운 없을 때만 onset 발화
  if (_energyRise > 0.10 && _poolOnsetCool === 0) {
    _poolOnsetStrength = Math.min(1.0, _energyRise * 3.5 + S.bass * 0.6);
    _poolOnsetCool = 40; // ~0.66초 쿨다운
  }
  // onset 강도 감쇠 (쫀득하게 오래 남음)
  _poolOnsetStrength *= 0.94;
}
...
function drawOverlay(){
  ...
  HLINES.forEach(l=>{
    ...
    const al=(0.016+bv*0.085+lqVal*0.055)*(0.5+0.5*Math.sin(ovTime*l.speed*2+l.phase))*palFade;
    if(al<0.004) return;
    const y=l.y*H;
    const waMid=LQ.midDensity*H*0.014;
    const waHigh=LQ.surfaceTension*H*0.006*Math.sin(ovTime*8+l.phase);
    const wa=(bv*H*0.012)+waMid+waHigh;
    ovCtx.beginPath();
    ovCtx.strokeStyle=`rgba(${pr},${pg},${pb},${al.toFixed(3)})`;
    ovCtx.lineWidth=l.thickness*(0.8+LQ.midDensity*0.4);
    for(let x=0;x<=W;x+=3){
      const wBase=wa*Math.sin(x/W*Math.PI*6+ovTime*l.speed*3+l.phase);
      const wShim=LQ.highShimmer*H*0.003*Math.sin(x/W*Math.PI*22+ovTime*14+l.phase*2);
  ...
  if(S.beat>0.5&&(beatRings.length===0||beatRings[beatRings.length-1].age>8))
    beatRings.push({cx,cy,r:0,maxR:Math.max(W,H)*0.72,age:0,strength:S.beat*1.0});
  ...
  if(sp>0.025) VLINES.forEach(l=>{
    const al=sp*0.072*(0.5+0.5*Math.sin(ovTime*l.speed+l.phase))*palFade;
    if(al<0.004) return;
    const x=l.x*W;
    const wa=sp*W*0.006+LQ.bassShock*W*0.003;
    ovCtx.beginPath();
    ovCtx.strokeStyle=`rgba(${pr},${pg},${pb},${al.toFixed(3)})`;
    ovCtx.lineWidth=0.6;
    for(let y=0;y<=H;y+=4){
  ...
  if(analyser&&timeData&&S.energy>0.012){
    ...
    const wfAl=(0.14+S.energy*0.32+LQ.midDensity*0.16)*palFade;
    const wfY=H*0.89, wfAmp=H*0.058*(0.4+S.energy*0.7), step=W/timeData.length;
    ovCtx.beginPath();
...
function renderChronEntry(entry){
  ...
  if(!pers.isStatic){
    if(pers.hoodieMode){
      ...
      model.rotation.y += pers.bRot * (1.2 + (S.bass + LQ.midDensity) * 1.6 + speakerPulse * 0.6);
      model.rotation.x = 0;
      model.rotation.z = 0;
      if(entry._baseModelY==null) entry._baseModelY = model.position.y;
      ...
      if(glbFile==="diamond_bape_buckle.glb" || pers.bapeMode){
        ...
        if(pers.oneWay){
          model.rotation.y+=pers.bRot*(0.8+bandVal*2.0+speakerPulse*0.6);
        } else {
          model.rotation.y+=pers.bRot*(0.6+bandVal*1.8+speakerPulse*0.5)*sign;
        }
...
function stepFluidSim(delta){
  ...
  const c = 0.30;
  // 쫀득: 평소엔 감쇠를 높게, onset 순간에 탱탱하게 살아있게
  const damp = 0.994 - _poolOnsetStrength * 0.006;

  // ── onset 임펄스 — 도입부/조용→큰소리 순간만 반동 ──
  if (_poolOnsetStrength > 0.04) {
    const str = _poolOnsetStrength;
    // 여러 지점에서 동시에 터져 파동이 퍼지게 (쫀득한 느낌)
    const spots = [
      { fx: 0.5,  fy: 0.5  },  // 중앙
      { fx: 0.25, fy: 0.35 },  // 좌상
      { fx: 0.75, fy: 0.65 },  // 우하
    ...
    spots.forEach(sp => {
      ...
      const r  = Math.floor(N * (0.06 + str * 0.10));
      const amp = str * 0.38;
      for (let dy=-r; dy<=r; dy++) for (let dx=-r; dx<=r; dx++) {
        ...
        fluidH0[yi*N+xi] += amp * (1 - d/r) * (1 - d/r); // 부드러운 falloff
      }
    });
  }

  // bass shock — 강한 저음 타격 (도입부 킥드럼 등)
  if (LQ.bassShock > 0.18) {
    const cx = Math.floor(N * 0.5), cy = Math.floor(N * 0.5);
    const r = Math.floor(N * 0.14);
    for(let dy=-r;dy<=r;dy++) for(let dx=-r;dx<=r;dx++){
      const d = Math.sqrt(dx*dx+dy*dy);
      if(d>r) continue;
      const xi=cx+dx, yi=cy+dy;
      if(xi<0||xi>=N||yi<0||yi>=N) continue;
      fluidH0[yi*N+xi] += LQ.bassShock * 0.22 * (1-d/r) * (1-d/r);
    }
  }
...
function updateWaterSurface(delta){
  ...
  // 유체 시뮬 반영 강도 — 음악 반응
  const fluidStrength = 1.05 + LQ.subPressure * 2.8 + poolMouseSpeed * 2.2 + S.beat * 0.72;
  if(u['uFluidStrength']) u['uFluidStrength'].value += (fluidStrength - u['uFluidStrength'].value) * 0.18;

  if(u['uSub']) u['uSub'].value = S.sub;
  if(u['uMid']) u['uMid'].value = S.mid;
  if(u['uHigh']) u['uHigh'].value = LQ.highShimmer;
  if(u['uBeat']) u['uBeat'].value = Math.min(1, S.beat + LQ.bassShock * 0.5);
